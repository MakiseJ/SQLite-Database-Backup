...
CREATE TABLE `give_course` (s_id VARCHAR(4), c_id VARCHAR(3), PRIMARY KEY (c_id, s_id), FOREIGN KEY (c_id) REFERENCES courses(c_id), FOREIGN KEY (s_id) REFERENCES staff(s_id)) ;
CREATE TABLE `work_on` (s_id VARCHAR(4), p_id VARCHAR(10), start_date INT, stop_date INT, PRIMARY KEY (p_id, s_id), FOREIGN KEY (p_id) REFERENCES projects(p_id), FOREIGN KEY (s_id) REFERENCES staff(s_id)) ;
